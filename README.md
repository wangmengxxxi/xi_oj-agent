...
- 动态学习路径推荐：Agent 可调用 `recommend_learning_path`，基于用户提交 AC 率、失败次数、错题标签和 RAG 知识点召回，自动识别薄弱知识点并推荐复习错题和后续练习题。
...
当用户询问“我该练什么”“哪里薄弱”“推荐学习路径”时，Agent 会优先调用 `recommend_learning_path`，按知识点 AC 率和失败次数识别薄弱环节，结合 RAG 知识点回顾、未通过题目和平台题库生成分层练习建议。

![AI Agent 动态学习路径推荐](doc/screenshots/11-recommend-learning-path.png)
...
| `recommend_learning_path` | 基于提交掌握度、错题标签和 RAG 知识点生成个性化学习路径 |

advanced Agent Loop 的特色：

- 自己控制推理循环，不完全依赖框架黑盒调度。
- 每一步只允许调用一个工具，观察结果后再进入下一轮推理。
- 当用户询问学习路径、练习推荐或薄弱点分析时，优先调用 `recommend_learning_path`，避免把诊断、知识检索和题目推荐拆散成多轮工具调用。
...
`recommend_learning_path` 是面向错题复盘和持续训练的聚合型 Agent 工具。它不会只列出错题，而是先读取当前用户的提交掌握度，按标签统计提交次数、AC 次数、失败次数和 AC 率，选择最薄弱的 3 个知识点作为学习路径主线。

处理流程：

```text
用户询问练习方向 / 薄弱点
  -> IntentRouter / Agent Prompt 优先选择 recommend_learning_path
  -> selectTagMastery 统计标签掌握度
  -> 读取 AI 错题本，匹配薄弱标签下的待复习错题
  -> RAG 按知识点召回基础知识和典型模式
  -> 题库中筛选用户尚未 AC 的同标签题目
  -> 输出知识点回顾、推荐练习和错题复习清单
```

推荐结果会按薄弱程度排序，每个知识点包含：

- AC 率和失败次数，用于解释为什么推荐这个方向。
- RAG 召回的知识点回顾，帮助先补概念再刷题。
- 平台真实题目链接，优先过滤用户已经 AC 的题目。
- 同标签历史错题，提示用户先复盘再迁移到新题。

配套 Prompt 补丁位于 `sql/recommend_learning_path_prompt.sql`，用于把第 12 个工具加入 `ai.prompt.agent_system`，并补充 AiServices 模式下的 `ai.prompt.chat_system` 工具说明和调用规范。
...
```text
用户问题
  -> Query Rewrite 兜底式最小改写
  -> 保留原始 query + rewritten query
  -> 分别 Embedding 向量化
...
- `QueryRewriter`：定位是“检索兜底”而不是主动扩写。它只对短 query 做最小化改写，用来修正不恰当或错误的表达，例如展开 `dp`、`bfs`、`dfs` 等缩写，纠正明显错别字，或把“二分”补全为“二分查找”。改写不会主动追加大量关键词，也不会把用户问题发散到原语义范围之外，并使用 Redis 缓存 120 分钟。
- `QueryRewriteTransformer`：不会丢弃原始 query；当 rewritten query 与原文不同时，会同时保留原始 query 和 rewritten query，让两路 query 都参与知识库召回。原始 query 负责保住用户真实意图，rewrite query 负责在用户表达不规范时补一条更容易命中向量库的检索路径，避免单一路径因为误写、缩写或口语化表达导致召回失败。
- `RerankingContentAggregator`：合并原始 query 与 rewritten query 在 `oj_knowledge`、`oj_question` 中召回的候选内容，按文本去重后再进入精排。
- `RerankService`：调用 DashScope `gte-rerank` 对候选片段重排，精排 query 优先使用用户原始 query，提升短 query 和语义相近场景的准确率。
...
sql/recommend_learning_path_prompt.sql # recommend_learning_path 工具 Prompt 补丁
```
