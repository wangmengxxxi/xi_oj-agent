...
更新时间：2026-04-29
适用对象：熟悉本项目单体架构的开发者，在单体项目中引入 RabbitMQ 实现判题链路的异步解耦
...
本文档是 XI OJ 平台在**单体架构下**引入 **RabbitMQ** 实现判题链路异步解耦的完整实施指南。覆盖以下内容：
...
本文档适用于**当前单体阶段**。虽然项目未拆分微服务，但判题链路存在以下痛点，适合在单体内引入 MQ 解耦：

- `CompletableFuture.runAsync()` 使用 ForkJoinPool 公共池，无消息持久化，服务重启丢失任务。
- 高并发提交时无削峰能力，沙箱服务容易被瞬时流量打垮。
- 无自动重试机制，沙箱超时或网络抖动需要用户手动重新提交。
- 为后续微服务拆分做好架构准备，届时只需将消费者迁移到独立服务即可。

### 1.3 不在本文档范围内

- AI 问答、代码分析等 AIGC 链路的 MQ 改造（这些链路通过 `@Async` 线程池已足够）。
- WebSocket 实时推送（当前轮询方案在判题场景下延迟可接受）。
- 微服务拆分（本文档聚焦单体架构下的 MQ 集成，微服务迁移仅在第九章简要提及演进路径）。
...
| **无法跨进程** | `CompletableFuture` 只能在同一 JVM 内执行 | 后续拆微服务时需要重新改造 |
...
| 场景 | 是否引入 MQ | 理由 |
|------|-------------|------|
| 用户提交代码判题 | 是 | 耗时 CPU 密集操作，需要削峰、持久化、自动重试 |
...
> "虽然是单体项目，但判题是耗时的 CPU 密集操作，用 MQ 做异步解耦有三个好处：一是**削峰**，高并发提交时判题逻辑不会被打垮；二是**持久化**，服务重启后消息不丢，不会出现提交'消失'的问题；三是**重试**，沙箱执行超时可以自动重新投递。其他模块用 `@Async` 线程池就够了，因为调用链路短、对实时性要求高，没必要引入 MQ 增加复杂度。而且单体阶段先把 MQ 跑通，后续拆微服务时判题消费者直接迁移到独立服务就行，改动量很小。"
...
当前单体阶段已通过 MQ 完成判题解耦。后续拆微服务时改动量很小：

| 链路 | 当前单体阶段 | 微服务阶段 |
|------|-------------|------------|
| 用户提交判题 | MQ 异步（生产者 + 消费者在同一 JVM） | 消费者迁移到独立判题服务（MQ 拓扑不变） |
| AI 工具判题 | `judgeService.doJudge()` 直接调用 | 替换为 **Feign Client** 同步调用判题服务 |

`AiJudgeService` 接口设计已预留了微服务迁移路径：
