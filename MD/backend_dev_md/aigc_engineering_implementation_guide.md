...
|---|---|---|---|---|---|---|
| `OJChatAgent` | AI 问答 | ✅ 多轮记忆（Redis+MySQL） | ✅ 自动注入 | ✅ 11个工具 | ✅ | 交互式编程问答（simple 模式） |
| `AgentLoopService` | AI 问答（advanced） | ❌（由 Service 层管理） | ✅ 自行检索（QueryRewrite+Rerank） | ✅ 11个工具（ToolDispatcher） | ✅ 流式（StreamingChatModel） | 自定义 ReAct 推理循环 + 可观测性 |
...
4. `buildRetrievalAugmentor()` 的 `topK`、`minScore` 从配置中心动态读取，`queryTransformer`（Query Rewrite）和 `contentAggregator`（Rerank）均可动态开关。
5. 所有消费方（Service 层）注入 `AiModelHolder` 而非直接注入模型 Bean，通过 getter 获取最新实例。
6. `AgentLoopService` 自带 RAG pipeline（不依赖 AiServices 的 `RetrievalAugmentor`），流式场景用 `CountDownLatch` 桥接 `StreamingChatResponseHandler` 异步回调到同步 ReAct 循环。
7. Advanced 模式的 `chatStream()` 用虚拟线程包装 `Flux.create()`，避免阻塞 Reactor 线程。
...
│
├── QueryTransformer（查询改写，已启用 — QueryRewriteTransformer）
│
...
private RetrievalAugmentor buildRetrievalAugmentor() {
    ...
    return DefaultRetrievalAugmentor.builder()
            .queryTransformer(queryRewriteTransformer)  // Query Rewrite（已启用）
            .contentAggregator(rerankingContentAggregator)  // Rerank（已启用，可配置关闭）
            .queryRouter(new DefaultQueryRouter(knowledgeRetriever, questionRetriever))
            .build();
}
```

> **注意**：`queryTransformer` 和 `contentAggregator` 是后续新增的 RAG 优化组件。`QueryRewriteTransformer` 实现 `QueryTransformer` 接口，用轻量模型改写用户 query；`RerankingContentAggregator` 实现 `ContentAggregator` 接口，调用 DashScope gte-rerank 模型精排。两者均可通过 `ai_config` 表动态开关。

> **Advanced 模式补充**：`AgentLoopService` 不使用此 `RetrievalAugmentor`，而是自行调用 `QueryRewriter` → `OJKnowledgeRetriever` → `RerankService` 完成检索，将结果作为 `UserMessage` 注入对话历史。

Agent 构建时绑定（simple 模式）：
...
| 状态事件 | `event: status` + `data: {"d":"..."}` | Agent 中间步骤状态（思考中、调用工具等） |
...
> **`[STATUS]` 前缀约定**：`AgentLoopService.runStreaming()` 在 ReAct 循环中通过 `sink.next("[STATUS]正在思考...")` 推送中间状态。Controller 检测到 `[STATUS]` 前缀后转为 `event: status` SSE 事件，前端 `sse.ts` 的 `onStatus` 回调接收并显示为加载指示器。

工程价值：
1. 前端只需一个 SSE 解析器即可复用所有 AI 流式接口（问答/代码分析/题目解析/错题分析）。
2. 错误事件使用独立 `event` 类型，前端可区分正常数据和异常。
3. `done` 事件明确标记流结束，前端无需依赖 SSE 连接关闭来判断完成。
4. `status` 事件让前端在 Agent 思考期间展示实时进度，提升用户体验。
...
**XI OJ 的做法：**
- 所有 AI 流式接口统一输出四种事件：`{"d":"token"}`（普通 token）、`event:status + {"d":"msg"}`（Agent 状态）、`{"done":true}`（结束）、`event:error + {"error":"msg"}`（错误）。
- Controller 层统一封装 `Flux<ServerSentEvent<String>>`，检测 `[STATUS]` 前缀自动转为 `event: status`。

**工程价值：** 前端一个 SSE 解析器复用所有 AI 流式接口，降低联调成本。Agent 思考过程通过 status 事件实时反馈给用户。
...
| 优化项 | 描述 | 优先级 | 状态 |
|---|---|---|---|
| Rerank 重排序 | 在向量检索后增加 Cross-Encoder 重排序（DashScope gte-rerank），提升 TopK 质量 | P0 | ✅ 已完成 |
| Query Rewrite | 用户 query 改写/扩展，提高召回鲁棒性（`QueryRewriteTransformer` + `QueryRewriter`） | P1 | ✅ 已完成 |
| `algorithm_type` metadata | 新增算法类型标签（如 DP/贪心/图论），支持更精准的类型过滤 | P1 | 待开发 |
| 离线评估体系 | 建立 Recall@K、MRR 等指标的离线评估集 | P2 | 待开发 |
| 上下文 token 预算器 | RAG 上下文裁剪/摘要，控制注入 Prompt 的 token 量 | P1 | 待开发 |
...
| 优化项 | 描述 | 优先级 | 状态 |
|---|---|---|---|
| AI 监控看板 | 时延、token 消耗、缓存命中率、错误率、限流触发率 | P0 | 待开发 |
| Agent 链路追踪 | 每次 Agent 调用的完整链路（Thought → Tool → Observation → Answer），每步记录到 `ai_agent_trace_log` 表 | P1 | ✅ 已完成 |
| 异常告警 | 模型调用失败率超阈值时自动告警 | P1 | 待开发 |
...
1. 架构层面：`AiModelHolder` 模型动态管理与即时重建、双向量库隔离、Agent/Service 分层清晰、自定义 Agent Loop（`AgentLoopService`）+ AiServices 双模式灰度切换。
2. 性能层面：RAG 缓存、配置缓存、记忆缓存三级 Redis 缓存体系。RAG 优化：Query Rewrite + Cross-Encoder Rerank。
3. 安全层面：全局开关、分层限流、Prompt 乱码防护、缓存穿透防护、AI 判题隔离。
4. 可维护性：配置中心动态治理、SSE 统一协议（含 status 事件）、游标分页、知识库全生命周期管理、Agent 决策链路日志。
5. 业务创新：自动错题收集、艾宾浩斯复习机制、ReAct 工具调用（11 个工具）、相似题推荐、题目页上下文对话小窗、Agent 流式推送。

后续迭代建议始终围绕两条主线：

1. 回答质量：RAG 精度（离线评估体系）+ 上下文治理（token 预算 + 混合记忆）。
2. 系统稳定：可观测性看板 + 异常告警 + 增量向量同步。
